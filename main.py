...
class Settings:
    ...
    def __init__(self):
        ...
        # Security: Prevent starting with default key in production if auth is required
        if self.REQUIRE_API_KEY and self.API_KEY == "default_secret_key":
            if self.DEPLOYMENT_ENV == "production":
                raise ValueError("API_KEY must be set in production when REQUIRE_API_KEY is true")
            else:
                logger.warning("Using default API key in non-production environment. This is insecure!")
